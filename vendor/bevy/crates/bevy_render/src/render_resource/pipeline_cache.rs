    ...
    fn start_create_render_pipeline(
        ...
        create_pipeline_task(
            async move {
                ...
                // 1. Collect the layouts into a stable Vec first
                let vertex_buffer_layouts: Vec<Option<RawVertexBufferLayout>> = descriptor
                    .vertex
                    .buffers
                    .iter()
                    .map(|layout| Some(RawVertexBufferLayout {
                        array_stride: layout.array_stride,
                        // Don't clone. Reference the attributes already owned by the 'layout'
                        attributes: &layout.attributes,
                        step_mode: layout.step_mode,
                    }))
                    .collect();

                // 2. Build the RawVertexState using the slice of the Vec
                let vertex_state = RawVertexState {
