...
use std::sync::Arc;
use wgpu::{Queue, SurfaceConfiguration, SurfaceTargetUnsafe, TextureFormat, TextureUsages, TextureViewDescriptor};
...
impl ExtractedWindow {
    ...
    pub fn present(&mut self, render_queue: Arc<WgpuWrapper<Queue>>) {
        if let Some(surface_texture) = self.swap_chain_texture.take() {
            ...
            surface_texture.present(render_queue.clone());
        }
    }
}
