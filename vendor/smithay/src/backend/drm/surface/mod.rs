...
impl DrmSurface {
    ...
    /// Set the DPMS power state of this surface's connectors. `true` powers the
    /// display on, `false` blanks it (backlight off, no scanout).
    ///
    /// Implemented via the legacy connector "DPMS" property, which drivers honor
    /// in both legacy and atomic mode. Recover from an off state with a forced
    /// modeset (`DrmDevice::activate(true)` + surface reset) — the resume
    /// watchdog path already performs exactly that.
    pub fn set_dpms(&self, on: bool) -> Result<(), Error> {
        super::device::legacy::set_connector_state(
            self.device_fd(),
            self.current_connectors().into_iter(),
            on,
        )
    }
