...
pub fn input_received<I: InputBackend>(
    ...
    // Natural scroll is handled at the libinput device level (wired from
    // preference). Compositor-level inversion would double-invert, so the
    // axis amounts are passed through as-is.
