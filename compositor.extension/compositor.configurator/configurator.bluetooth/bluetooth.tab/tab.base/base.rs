...
pub fn build<'a>(b: &'a BtSnapshot) -> Element<'a, SettingsMessage, Theme, Renderer> {
    let header = row![
        ...
        button(text(if b.powered { "On" } else { "Off" })).on_press(SettingsMessage::BtPower(!b.powered)).style(style::action),
    ].spacing(8);
    ...
    for d in &b.devices {
        ...
        rows.push(
            row![text(format!("{name}{status}")).width(Length::Fill), button(text(lbl)).on_press(msg).style(style::action)]
                .spacing(8)
                .into(),
        );
    }
