...
use crate::{ScaleConfig, XResolution};
        ...
        match self {
            ...
            SurfaceEvents::FractionalScale(event) => match event {
                wp_fractional_scale_v1::Event::PreferredScale { scale } => {
                    ...
                    let log = if state.scale_config.log_scale { log::Level::Info } else { log::Level::Debug };
                    log::log!(log, "{} fractional preferred_scale: {factor:.3}",
...
impl SurfaceEvents {
    ...
    fn xdg_event<C: XConnection>(
        ...
        if let Some(pending) = pending {
            ...
            update_surface_viewport(&state.world, state.world.query_one(target).unwrap(), &state.scale_config);
        }
...
pub(super) fn update_surface_viewport(
    ...
    // --force-scale overrides the per-surface scale; --identity-viewport skips the division entirely.
    // BUG NOTE: Without flags, DPI-unaware apps (fixed pixel size, no Xft.dpi response) shrink as
    // scale increases because their X dims stay constant while this division grows.
    // The offending computation is right here — dims / scale_factor produces a smaller logical
    // destination as scale_factor increases, even though dims didn't change.
    let effective_scale = scale_config.force_scale.unwrap_or(scale_factor.0);
    let (width, height) = if scale_config.identity_viewport {
        // --identity-viewport: present buffer at its native X pixel size, no scale division
        (dims.width as i32, dims.height as i32)
    } else {
        (
            (dims.width as f64 / effective_scale).ceil() as i32,
            (dims.height as f64 / effective_scale).ceil() as i32,
        )
    };
    // Guard against zero/negative viewport size (would panic the compositor)
    if width > 0 && height > 0 {
        viewport.set_destination(width, height);
    }

    let log = if scale_config.log_scale { log::Level::Info } else { log::Level::Debug };
    log::log!(log, "{} viewport: X={}x{} effective_scale={:.3} dst={}x{}",
        surface.id(), dims.width, dims.height, effective_scale, width, height);
    ...
    {
        d.draw_decorations(world, width, effective_scale as f32);
    }
    ...
    if let Some(hints) = size_hints {
        ...
        // Use 1.0 divisor for --identity-viewport so size hints match the no-division viewport
        let size_scale = if scale_config.identity_viewport { 1.0 } else { effective_scale };

        if let Some(min) = hints.min_size {
            data.toplevel.set_min_size(
                (min.width as f64 / size_scale) as i32,
                (min.height as f64 / size_scale) as i32 + decorations_height,
            );
        }
        if let Some(max) = hints.max_size {
            data.toplevel.set_max_size(
                (max.width as f64 / size_scale) as i32,
                (max.height as f64 / size_scale) as i32 + decorations_height,
            );
        }
    }
}
...
impl OutputEvent {
    fn wl_event<C: XConnection>(
        ...
        match event {
            ...
            Event::Scale { factor } => {
                let log = if state.scale_config.log_scale { log::Level::Info } else { log::Level::Debug };
                log::log!(log, "{} integer scale: {factor}", state.world.get::<&WlOutput>(target).unwrap().id());
                if update_output_scale(
                    state.world.query_one(target).unwrap(),
    ...
    fn xdg_event<C: XConnection>(
        ...
        match event {
            ...
            // --x-resolution: choose physical (current/default) or logical (from xdg_output event)
            Event::LogicalSize { width: logical_w, height: logical_h } => {
                // Extract config before the mutable world borrow
                let use_logical = state.scale_config.x_resolution == XResolution::Logical;
                let log = if state.scale_config.log_scale { log::Level::Info } else { log::Level::Debug };
                ...
                let (out_w, out_h) = if use_logical {
                    if dimensions.rotated_90 { (logical_h, logical_w) } else { (logical_w, logical_h) }
                } else {
                    if dimensions.rotated_90 { (dimensions.height, dimensions.width) } else { (dimensions.width, dimensions.height) }
                };
                log::log!(log, "Xwayland screen resolution: {out_w}x{out_h} (physical={}x{}, logical={}x{})",
                    dimensions.width, dimensions.height, logical_w, logical_h);
                xdg.logical_size(out_w, out_h);
            }
            _ => simple_event_shunt! {
