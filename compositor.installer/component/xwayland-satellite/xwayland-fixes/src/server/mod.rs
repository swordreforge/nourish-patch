...
use crate::{ScaleConfig, X11Selection, XConnection, timespec_from_millis};
...
impl<S: X11Selection> ServerState<NoConnection<S>> {
    pub fn new(
        ...
        // --ignore-fractional-scale: skip binding so only integer wl_output scale is used
        let fractional_scale = if scale_config.ignore_fractional_scale {
            None
        } else {
            global_list
...
impl<C: XConnection> ServerState<C> {
    /// Whether `--popup-fix` is enabled (see ScaleConfig::popup_fix). Read by the
    /// X11 side so the popup-vs-toplevel heuristic can opt into the fix.
    pub fn popup_fix(&self) -> bool {
        self.inner.scale_config.popup_fix
    }
...
impl<S: X11Selection + 'static> InnerServerState<S> {
    ...
    pub fn reconfigure_window(&mut self, event: x::ConfigureNotifyEvent) {
        ...
        // --force-scale overrides the per-surface scale for positioner geometry
        let effective_scale = self.scale_config.force_scale.unwrap_or(scale_factor.0);
        match role {
            SurfaceRole::Popup(Some(popup)) => {
                let (px, py, pw, ph) = if self.scale_config.identity_viewport {
                    (
                        (event.x() as i32 - win.output_offset.x),
                        (event.y() as i32 - win.output_offset.y),
                        1.max(event.width() as i32),
                        1.max(event.height() as i32),
                    )
                } else {
                    (
                        ((event.x() as i32 - win.output_offset.x) as f64 / effective_scale) as i32,
                        ((event.y() as i32 - win.output_offset.y) as f64 / effective_scale) as i32,
                        1.max((event.width() as f64 / effective_scale) as i32),
                        1.max((event.height() as f64 / effective_scale) as i32),
                    )
                };
                popup.positioner.set_offset(px, py);
                popup.positioner.set_size(pw, ph);
            ...
            SurfaceRole::Toplevel(Some(_)) => {
                ...
                drop(query);
                drop(win);
                update_surface_viewport(&self.world, self.world.query_one(data.entity()).unwrap(), &self.scale_config);
            }
    ...
    fn create_popup(&mut self, entity: Entity, xdg: XdgSurface, parent: x::Window) -> PopupData {
        ...
        // --force-scale overrides the effective scale for positioner geometry
        let effective_scale = self.scale_config.force_scale.unwrap_or(initial_scale);
        *scale = *parent_scale;
        ...
        let (pw, ph, px, py, par_w, par_h) = if self.scale_config.identity_viewport {
            (
                1.max(window.attrs.dims.width as i32),
                1.max(window.attrs.dims.height as i32),
                (window.attrs.dims.x - parent_dims.x) as i32,
                (window.attrs.dims.y - parent_dims.y) as i32,
                parent_window.attrs.dims.width as i32,
                parent_window.attrs.dims.height as i32,
            )
        } else {
            (
                1.max((window.attrs.dims.width as f64 / effective_scale) as i32),
                1.max((window.attrs.dims.height as f64 / effective_scale) as i32),
                ((window.attrs.dims.x - parent_dims.x) as f64 / effective_scale) as i32,
                ((window.attrs.dims.y - parent_dims.y) as f64 / effective_scale) as i32,
                (parent_window.attrs.dims.width as f64 / effective_scale) as i32,
                (parent_window.attrs.dims.height as f64 / effective_scale) as i32,
            )
        };
        positioner.set_size(pw, ph);
        let x = px;
        let y = py;
        positioner.set_offset(x, y);
        positioner.set_anchor(Anchor::TopLeft);
        positioner.set_gravity(Gravity::BottomRight);
        positioner.set_anchor_rect(0, 0, par_w, par_h);
        positioner
