...
use xwayland_satellite::{ScaleConfig, XResolution};
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "-help" => {
                ...
                println!();
                println!("HiDPI/scaling diagnostic flags (all default to current behavior when unset):");
                println!("{:<35} override effective scale S with constant (1.0 = no scaling)", "--force-scale <f64>");
                println!("{:<35} use only integer wl_output scale, skip wp_fractional_scale", "--ignore-fractional-scale");
                println!("{:<35} set wp_viewport destination to native X pixel size (no scale division)", "--identity-viewport");
                println!("{:<35} do not write Xsettings DPI; cede to external xsettingsd", "--no-xsettings-dpi");
                println!("{:<35} screen dimensions advertised to Xwayland (logical|physical)", "--x-resolution <logical|physical>");
                println!("{:<35} log scale/viewport info at INFO level on each output change and configure", "--log-scale");
                println!("{:<35} keep borderless CSD windows with WM_DELETE_WINDOW as toplevels, not popups (fixes Isaac Sim tear-off panels)", "--popup-fix");
                std::process::exit(0);
            }
            "-listen" => {
            ...
            "--force-scale" => {
                let val: f64 = args
                    .next()
                    .expect("argument to --force-scale not provided")
                    .parse()
                    .expect("argument to --force-scale must be a float (e.g. 1.0)");
                data.scale_config.force_scale = Some(val);
            }
            "--ignore-fractional-scale" => {
                data.scale_config.ignore_fractional_scale = true;
            }
            "--identity-viewport" => {
                data.scale_config.identity_viewport = true;
            }
            "--no-xsettings-dpi" => {
                data.scale_config.no_xsettings_dpi = true;
            }
            "--x-resolution" => {
                let val = args.next().expect("argument to --x-resolution not provided");
                data.scale_config.x_resolution = match val.as_str() {
                    "logical" => XResolution::Logical,
                    "physical" => XResolution::Physical,
                    other => panic!("unknown value for --x-resolution: {other} (expected 'logical' or 'physical')"),
                };
            }
            "--log-scale" => {
                data.scale_config.log_scale = true;
            }
            "--popup-fix" => {
                data.scale_config.popup_fix = true;
            }
            _ => {
