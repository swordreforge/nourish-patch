...
impl XState {
    ...
    fn guess_is_popup(
        ...
        // --popup-fix: a window that registers WM_DELETE_WINDOW is an independent,
        // closeable toplevel: genuine menus/tooltips/dropdowns/DnD popups never
        // request it. Don't let the softer CSD (MOTIF) / skip-taskbar heuristics
        // below demote such a window to an xdg_popup, which renders as an empty
        // surface on the host (e.g. Isaac Sim's torn-off "Layer"/"Render Settings"
        // panels). override_redirect and explicit popup window types are still
        // honored. Gated behind --popup-fix; off by default this is a no-op.
        let delete_window_present = protocols.resolve()?.is_some_and(|reply| {
            reply
                .value::<x::Atom>()
                .contains(&self.atoms.wm_delete_window)
        });
        let has_delete_window = popup_fix && delete_window_present;
        if let Some(hints) = motif_hints {
            // If MOTIF_WM_HINTS provides no decorations for client assume its a popup
            // (unless the window is a managed closeable toplevel — see has_delete_window).
            motif_popup = !has_delete_window && hints.decorations.is_some_and(|d| d.is_clientside());
            ...
            if !has_delete_window && hints.functions.is_some_and(|f| f.is_empty()) {
                return Ok(true);
            }
        }
        ...
        if !known_window_type {
            if let Some(has_skip_taskbar) = has_skip_taskbar {
                is_popup = (has_skip_taskbar && !has_delete_window) || override_redirect;
            }
        }
